...
class RBY1Robot(ManipulatedRobot):
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[3*self.num_actions:4*self.num_actions] = 0. # previous actions
    ...
    def _init_target(self):
        ...
        self.targets_0 = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
        self.targets_1 = torch.zeros(self.num_envs, self.num_dof, dtype=torch.float, device=self.device, requires_grad=False)
        ...
        for i in range(14):
            ...
            self.targets_0[i] = angle
        for i in range(14):
            angle = target_data.mot_target1[i+6]
            self.targets_1[i] = angle


        for i in range(14):
            angle = target_data.mot_target1[i+6]
            self.targets[i] = angle

        for i in range(6):
            #angle = target_data.hand_target0[i]
            angle = target_data.hand_target1[i]
    ...
    def check_termination(self):
        ...
        dist_to_target=torch.sum(torch.square(self.dof_pos[:, :] - self.targets_1), dim=1)
    ...
    def compute_observations(self):
        ...
        self.obs_buf = torch.cat((
                                    #(self.dof_pos - self.default_dof_pos - self.targets_1) * self.obs_scales.dof_pos,
                                    (self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos,
                                    self.dof_vel * self.obs_scales.dof_vel,
                                    (self.targets_1) * self.obs_scales.dof_pos,
        ...
        # add noise if needed
        #print('hand_pos : ',self.hand_pos[:]-self.hand_target)
        if self.add_noise:
            self.obs_buf += (2 * torch.rand_like(self.obs_buf) - 1) * self.noise_scale_vec

    def _reward_reaching(self):
        pos_error = torch.square(self.dof_pos[:, :] - self.targets_1)
        return torch.sum(pos_error, dim=(1))

    def _reward_approaching(self):
        pos_error = torch.square(self.dof_pos[:, :] - self.targets_1)
        last_pos_error = torch.square(self.last_dof_pos[:, :] - self.targets_1)
