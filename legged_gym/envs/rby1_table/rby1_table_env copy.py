...
ORANGE_ASSET_PATH = "./assets/urdf/ycb/017_orange.urdf"
DOOR_ASSET_PATH = "./assets/urdf/door.urdf"
urdf_path = os.path.abspath(DOOR_ASSET_PATH)
...
class RBY1Robot_table(ManipulatedRobot):
    def _create_envs(self):
        ...
        table_asset, _ = self._load_table_asset()
        orange_asset, _ = self._load_object_asset(ORANGE_ASSET_PATH, False)
        door_asset, _ = self._load_object_asset(DOOR_ASSET_PATH, True)

        self.robot_num_dof = self.gym.get_asset_dof_count(robot_asset)
        self.door_num_dof = self.gym.get_asset_dof_count(door_asset)
        self.total_num_dof = 0
        for asset in [robot_asset, door_asset]:
            self.total_num_dof += self.gym.get_asset_dof_count(asset)

        self.num_dof = self.robot_num_dof
        ...
        for i in range(self.num_envs):
            ...
            orange_pose.p = gymapi.Vec3(*pos)
            orange_pose.p += gymapi.Vec3(0.8, 0, 0.8 + 0.1)
            orange_pose.r = gymapi.Quat(0, 0, 0, 1)

            orange_handle = self.gym.create_actor(env_handle, orange_asset, orange_pose, f"orange_{i}", i, self.cfg.asset.self_collisions, 0)
            self.orange_handles.append(orange_handle)

            # Door
            door_pose = gymapi.Transform()
            door_pose.p = gymapi.Vec3(*pos) + gymapi.Vec3(0.7, 0.4, 0.1)
            door_pose.r = gymapi.Quat(0, 0, 1, 0)
            door_handle = self.gym.create_actor(env_handle, door_asset, door_pose, f"door_{i}", i, self.cfg.asset.self_collisions, 0)

            # door_dof_props = self.gym.get_asset_dof_properties(door_asset)
            # door_dof_props = self._process_dof_props(door_dof_props, i)
            ...
            # door_shape_props = self._process_rigid_shape_props(door_shape_props, i)
            # self.gym.set_actor_rigid_shape_properties(env_handle, door_handle, door_shape_props)

            # door_body_props = self.gym.get_actor_rigid_body_properties(env_handle, door_handle)
            # door_body_props = self._process_rigid_body_props(door_body_props, i)
            # self.gym.set_actor_rigid_body_properties(env_handle, door_handle, door_body_props, recomputeInertia=True)
    ...
    def _load_object_asset(self, object_asset_path, is_fix_base_link):
        asset_options = gymapi.AssetOptions()
        asset_options.fix_base_link = is_fix_base_link
    ...
    def _init_target(self):
        ...
        print("self.dof_target_pos.shape()", self.dof_target_pos.shape)
        ...
        print("self.dof_target_pos: ", self.dof_target_pos)
        for i in range(7):
    ...
    def compute_observations(self):
        self.obs_buf = torch.cat((
            self.dof_pos[:, :self.num_actions] * self.obs_scales.dof_pos,
    ...
    def _compute_torques(self, actions):
        ...
        robot_dof_pos = self.dof_pos[:, :self.num_actions]
        robot_dof_vel = self.dof_vel[:, :self.num_actions]

        torques = torch.zeros(self.num_envs, self.total_num_dof, device=self.device)
        if control_type == "P_none":
            robot_torques = self.p_gains * (actions_scaled - robot_dof_pos) - self.d_gains * robot_dof_vel
        elif control_type == "P_target":
            robot_torques = self.p_gains * (actions_scaled + self.default_dof_pos - robot_dof_pos - self.dof_target_pos[:, :self.num_actions]) - self.d_gains * robot_dof_vel
        elif control_type == "P":
            robot_torques = self.p_gains * (actions_scaled + self.default_dof_pos - robot_dof_pos) - self.d_gains * robot_dof_vel
        elif control_type == "V":
            robot_torques = self.p_gains * (actions_scaled - robot_dof_vel) - self.d_gains * (robot_dof_vel - self.last_dof_vel[:, :self.num_actions]) / self.sim_params.dt
        elif control_type == "T":
            robot_torques = actions_scaled
        else:
            raise NameError(f"Unknown controller type: {control_type}")

        torques[:, :self.robot_num_dof] = robot_torques

        robot_torque_limits = torch.tensor(self.torque_limits, device=self.device)  
        door_torque_limits = torch.zeros(2, device=self.device)        
        self.torque_limits = torch.cat([robot_torque_limits, door_torque_limits]) 
