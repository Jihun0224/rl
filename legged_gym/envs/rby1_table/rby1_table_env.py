...
class RBY1Robot_table(ManipulatedRobot):
    def _create_envs(self):
        ...
        self.num_robot_dof = self.gym.get_asset_dof_count(robot_asset)      # 14
        self.num_door_dof = self.gym.get_asset_dof_count(door_asset)        # 2
        self.num_dof = self.num_robot_dof + self.num_door_dof               # 16
    ...
    def compute_observations(self):
        self.obs_buf = torch.cat((
            self.dof_pos[:, :self.num_robot_dof] * self.obs_scales.dof_pos,
            self.dof_vel[:, :self.num_robot_dof] * self.obs_scales.dof_vel,
    ...
    def _compute_torques(self, actions):
        ...
        robot_dof_pos = self.dof_pos[:, :self.num_robot_dof]
        robot_dof_vel = self.dof_vel[:, :self.num_robot_dof]
        robot_torques = torch.zeros(self.num_envs, self.num_robot_dof, device=self.device)
        ...
        if control_type == "P_none":
            ...
            robot_torques = self.p_gains * (actions_scaled + self.default_dof_pos - robot_dof_pos - self.dof_target_pos[:, :self.num_robot_dof]) - self.d_gains * robot_dof_vel
        elif control_type == "P":
            robot_torques = self.p_gains * (actions_scaled + self.default_dof_pos - robot_dof_pos) - self.d_gains * robot_dof_vel
        elif control_type == "V":
            robot_torques = self.p_gains * (actions_scaled - robot_dof_vel) - self.d_gains * (robot_dof_vel - self.last_dof_vel[:, :self.num_robot_dof]) / self.sim_params.dt
        elif control_type == "T":
            robot_torques = actions_scaled
        else:
            raise NameError(f"Unknown controller type: {control_type}")

        torques = torch.zeros(self.num_envs, self.num_dof, device=self.device)
        torques[:, :self.num_robot_dof] = robot_torques  # door part is left as zero
