...
from .manipulated_robot_config import ManipulatedRobotCfg

import pdb

class ManipulatedRobot(BaseTask):
    def __init__(self, cfg: ManipulatedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.table_asset_file = "table.urdf"
        #print("Table asset file path:", self.table_asset_file)
    ...
    def step(self, actions):
        ...
        print("dof_pos(before):", self.dof_pos[0])
        print("torques(before):", self.torques[0])
        ...
        print("dof_pos(after):", self.dof_pos[0])
        print("torques(after):", self.torques[0])

        if self.privileged_obs_buf is not None:
            self.privileged_obs_buf = torch.clip(self.privileged_obs_buf, -clip_obs, clip_obs)
        return self.obs_buf, self.privileged_obs_buf, self.rew_buf, self.reset_buf, self.extras

    def get_robot_indices(self):
        robot_indices=[]
        for i in range(self.num_envs):
            robot_indices.append(self.gym.get_actor_index(self.envs[i], self.actor_handles[i], gymapi.DOMAIN_SIM))
    ...
    def post_physics_step(self):
        ...
        # prepare quantities
        self.base_pos[:] = self.root_states[self.robot_indices, 0:3]
    ...
    def _init_buffers(self):
        ...
        # 0530
        self.robot_indices = self.get_robot_indices()
        ...
        gravity_base = get_axis_params(-1., self.up_axis_idx)
        self.gravity_vec = to_torch(gravity_base, dtype=torch.float32, device=self.device).repeat(self.num_envs, 1)
        self.forward_vec = to_torch([1., 0., 0.], dtype=torch.float32, device=self.device).repeat(self.num_envs, 1)
        ...
        self.commands_scale = torch.tensor([self.obs_scales.lin_vel, self.obs_scales.lin_vel, self.obs_scales.ang_vel], dtype=torch.float32, device=self.device, requires_grad=False)  # TODO change this
        ...
        # print("base_quat.shape:", self.base_quat.shape)
        # print("gravity_vec.shape:", self.gravity_vec.shape)
        self.projected_gravity = quat_rotate_inverse(self.base_quat, self.gravity_vec)
        ...
        self.default_hand_pos = torch.zeros(self.cfg.env.num_hand_dof, dtype=torch.float, device=self.device, requires_grad=False)
        for i in range(self.cfg.env.num_hand_dof):
            h_angle=self.cfg.init_state.default_hand_angles[i]
            self.default_hand_pos[i]=h_angle
        self.default_hand_pos = self.default_hand_pos.unsqueeze(0)
