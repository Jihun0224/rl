...
class RBY1RoughCfg_finger( ManipulatedRobotCfg ):
    class init_state( ManipulatedRobotCfg.init_state ):
        ...
        default_joint_angles = { # = target angles [rad] when action = 0.0
           ...
           'gripper_finger_r1' : 0.5,                                             
           'gripper_finger_r2' : 0.5,                              
           'gripper_finger_l1' : 0.5,       
           'gripper_finger_l2' : 0.5,
    ...
    class env(ManipulatedRobotCfg.env):
        ...
        num_envs = 512
        #num_observations = 54
        #num_privileged_obs = 54     # + hand_pos 6
        #num_observations = 18
        #num_privileged_obs = 18     # hand_pos 6 + hand_vel 6 + hand_target 6
        num_observations = 36
        num_privileged_obs = 36     # hand_pos 6 + hand_vel 6 + hand_target 6 + acion 14
        #num_observations = 62
        #num_privileged_obs = 62     # + hand_pos 6
        #num_observations = 68
        #num_privileged_obs = 68     # + hand_pos&target 12
        #num_privileged_obs = 42     #
        num_actions = 18    # upper : 14, rby1 : 24
        episode_length_s = 1.0 # episode length in seconds
        num_hand_dof=6
        env_spacing=3
        num_actors_per_env = 3  # robot+table + bin
    ...
    class table:
        file = '{LEGGED_GYM_ROOT_DIR}/resources/objects/urdf/table.urdf'
        name = 'table'
        fix_base_link = True
    
    class asset( ManipulatedRobotCfg.asset ):
        #file = '{LEGGED_GYM_ROOT_DIR}/resources/robots/rby1/rby1_upper.urdf'
        file = '{LEGGED_GYM_ROOT_DIR}/resources/robots/rby1/rby1_upper_finger.urdf'
        name = "rby1_finger"
        foot_name = "wheel"
        hand_name = "arm_6"
        #penalize_contacts_on = [""]
        terminate_after_contacts_on = ["torso_5"]
    ...
    class rewards( ManipulatedRobotCfg.rewards ):
        ...
        class scales:
            ...
            dof_vel = -2e-4
            dof_acc = -2.0e-7
            target_pos_dist=-0.05
            target_hand_dist=-10.0
            action_rate = -0.0015
            wrist = -0.001
            arrival = 1.0

class RBY1RoughCfgPPO_finger( ManipulatedRobotCfgPPO ):
    class policy:
        init_noise_std = 0.8
