...
class RBY1Robot(ManipulatedRobot):
    ...
    def _get_noise_scale_vec(self, cfg):
        ...
        noise_vec[3*self.num_actions:3*self.num_actions+6] = 0.
        #noise_scales.hand_pos * noise_level * self.obs_scales.hand_pos
        noise_vec[3*self.num_actions+6:4*self.num_actions+6] = 0.
    ...
    def _init_target(self):
        ...
        for i in range(7):
            ...
            angle2 = target_data.mot_target[i+6+7]    # +6, omitting the torso joints
    ...
    def check_termination(self):
        ...
        dist_to_target=torch.sum(torch.square(self.dof_pos[:, :] - self.dof_target), dim=1)
    ...
    def compute_observations(self):
        ...
        self.obs_buf = torch.cat((
                                    #(self.dof_pos - self.default_dof_pos - self.dof_target) * self.obs_scales.dof_pos,
                                    (self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos,
                                    self.dof_vel * self.obs_scales.dof_vel,
                                    (self.dof_target) * self.obs_scales.dof_target,
                                    (self.hand_pos - self.hand_target_pos) * self.obs_scales.dof_pos,
                                    #(self.hand_pos) * self.obs_scales.hand_pos,
                                    #(self.hand_target) * self.obs_scales.dof_pos,
                                    self.actions,
                                    ),dim=-1)
    ...
    def _reward_to_target_pos(self):
    ...
    def _reward_to_target_hand(self):
